...
def V_def(state_shape):
	...
	outputs = layers.Dense(1, activation='relu')(dense2)
...
def actor_def(state_shape, action_shape):
	inputs = keras.Input(shape=state_shape)
	dense1 = layers.Dense(64, activation='sigmoid')(inputs)
	dense2 = layers.Dense(64)(dense1)
	outputs = layers.Dense(np.squeeze(action_shape))(dense2)
...
def full_model_def(f, actor, V, state_shape, args):
	...
	# loss_V_fxu = tf_geo_mean(tf.maximum(Vx - V_fxu + 0.1, 0), keepdims=True)
...
parser.add_argument("--epsilon_x", type=float,default=1.0)
