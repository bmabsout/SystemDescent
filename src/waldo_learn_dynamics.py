...
def discriminator_def(env: gym.Env, hidden_sizes:list[int], num_states: int):
    ...
    output = layers.Dense(num_states, activation="sigmoid")(flattened)
...
def generate_fakes(generator, batch):
    ...
    return ({
        ...
        "next_state": generator_2d_batch(generator, {
            **batch,
            "latent": tf.random.normal(shape=latent_shape) #don't forget this!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        }),
    })
...
def gan_dfls(generator, discriminator, real_batch):
    generate_fakes(generator, real_batch)
    generator_dfl = utils.DFL(0.0, {
        # "fooled_amount": utils.p_mean(discriminator(), 0.0,slack=1e-5),
        "direct": direct_dfls(generator, real_batch)
    })
    discriminator_dfl = utils.DFL(0.0, {
        "correct": utils.p_mean(discriminator(real_batch), 0.0),
    })
...
def train_GAN_step(generator, discriminator, learning_rate):
    ...
    def gradient_step(batch):
        ...
        # if train_discriminator and generator_scalar > 0.1:
...
def system_identify(env_name: str, generator_hidden_sizes: list[int], discriminator_hidden_sizes: list[int], batch_size: int, num_states: int, num_batches: int, epochs: int, episode_size: int, num_validation_batches: int,  save_freq: int, learning_rate: float, load_saved: Union[str, None]):
    ...
    data = dataset.take(num_batches).cache(filename='cache5.tmp')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--generator_hidden_sizes', nargs="+", type=int, default=[100,25])
    parser.add_argument('--discriminator_hidden_sizes', nargs="+", type=int, default=[20, 10])
