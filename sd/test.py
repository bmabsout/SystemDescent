...
def plot_lyapunov(lyapunov, actor, dynamics, set_point, fname, interactive=False):
        ...
        after = dynamics(
            {
                ...
                "latent": np.random.normal(
                    size=(inputs.shape[0],) + dynamics.input["latent"].shape[1:]
                ),
            },
            training=False,
        )
    ...
    def distance_to_setpoint(state, setpoint):
        return np.sqrt(
            (state[0, 0] - setpoint[0]) ** 2
            + (state[0, 1] - setpoint[1]) ** 2
            + (state[0, 2] - setpoint[2]) ** 2
        )
    ...
    def mouse_event(event):
        ...
        if event.button == 1:  # Left mouse button
            set_point = np.array(
                [np.cos(event.xdata), np.sin(event.xdata), event.ydata]
            )
            cur_setpoint = set_point
            ...
            print(
                f"Init state set to theta: {event.xdata} and theta_dot: {event.ydata}"
            )

            update_count = 0
            colors = cm.Reds(np.linspace(0, 1, MAX_UPDATES))
            while (
                distance_to_setpoint(state, cur_setpoint) > CLOSE_ENOUGH_THRESHOLD
                and update_count < MAX_UPDATES
            ):
                pt = (np.arctan2(state[0, 1], state[0, 0]), state[0, 2])
                if -np.pi < pt[0] < np.pi and -7 < pt[1] < 7:
                    scatter_plot = plt.scatter(
                        pt[0], pt[1], c=[colors[update_count]], s=10
                    )
                    ...
                    # plt.pause(0.0)
                act = actor(
                    {"state": state, "setpoint": np.array([cur_setpoint])},
                    training=False,
                )
                state = dynamics(
                    {
                        ...
                        "action": act,
                        "latent": np.random.normal(
                            size=(1,) + dynamics.input["latent"].shape[1:]
                        ),
                    },
                    training=False,
                )
    ...
    def run_test(num_steps=2000):
        ...
        def feed_obs(obs):
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                setpoint = angle_to_setpoint(
                    np.arctan2(*(np.array(surface1.get_size()) / 2.0 - cur_pos))
                )
