...
class DiffRobotEnv(ModelableEnv):
    ...
    *r = -(distance² + 0.1 * angle_error² + 0.001 * (v² + w²))*
    ...
    def __init__(self, render_mode: Optional[str] = None, max_vel=2.0, dt=0.05, screen=None):
    ...
    def step(self, action):
        ...
        distance_error = np.sqrt((new_x - self.target_x)**2 + (new_y - self.target_y)**2)
        angle_error = np.abs(np.arctan2(np.sin(new_theta - self.target_theta), 
    ...
    def render(self):
        ...
        gfxdraw.filled_circle(self.surf, target_screen_x, target_screen_y, 20, (0, 255, 0))
        ...
        if self.last_action is not None:
            ...
            if abs(v) > 0.01:  # Only draw if significant velocity
                ...
                pygame.draw.line(self.surf, (0, 0, 255), 
                               (screen_x, screen_y), (vel_end_x, vel_end_y), 3)
