...
class DistanceWrapper(ModelableWrapper):
    @staticmethod
    @tf.function
    def closeness_fpl(obs1: tf.Tensor, obs2: tf.Tensor) -> fpl.FPL:
        ...
        # tf.print(-fpl.p_mean(abs_diff, 1.0))
        # rpy_sims = fpl.geo(10.0/(10.0 + abs_diff), axis=2)
        normed_closeness = 1.0 - tf.abs(normed_obs1 - normed_obs2)

        # r = (1)
        # print(self.obs["ang_v"], "\t", r)
        return fpl.p_mean(fpl.smooth_constraint(normed_closeness, 0.5, 1.0), 0.0)
...
class MultiAttitudeEnv(gym.Env):
    ...
    def __init__(
        ...
        print(
            f"""[INFO] BaseAviary.__init__() loaded parameters from the drone's .urdf:
        ...
        if self.GUI:
            ...
            if self.USER_DEBUG:
                ...
                for i in range(4):
                    self.SLIDERS[i] = self.CLIENT.addUserDebugParameter(
                        "Propeller " + str(i) + " RPM", 0, self.MAX_RPM
                    )
                self.INPUT_SWITCH = self.CLIENT.addUserDebugParameter(
    ...
    def reset(self):
        ...
        for droneId, i in zip(self.DRONE_IDS, range(len(self.DRONE_IDS))):
            self.CLIENT.resetBasePositionAndOrientation(
                droneId, self.INIT_XYZS[i], self.quat[i]
            )
            self.CLIENT.resetBaseVelocity(
                droneId,
                self.vel[i],
                rpy_to_pybullet(self.obs["ang_v"][i] * self.DEG2RAD),
            )
        #### Update and store the drones kinematic information #####
        self.CLIENT.stepSimulation()
        self._updateAndStoreKinematicInformation()
    ...
    def step(self, action):
        ...
        if self.USE_GUI_RPM:
            for i in range(4):
                self.gui_input[i] = self.CLIENT.readUserDebugParameter(
                    int(self.SLIDERS[i])
                )
    ...
    def render(self, mode="human", close=False):
        ...
        for i in range(self.NUM_DRONES):
            ...
            print(
                f"[INFO] BaseAviary.render() ——— drone {i}",
    ...
    def _create_link_dicts(self):
        ...
        for drone_id in self.DRONE_IDS:
            link_dict = {
                self.CLIENT.getBodyInfo(drone_id): -1,
            }
    ...
    def _updateAndStoreKinematicInformation(self):
        ...
        for i in range(self.NUM_DRONES):
            ...
            original_pos, original_quat = self.CLIENT.getBasePositionAndOrientation(
                self.DRONE_IDS[i]
            )
            ...
            current_ang_v = (
                utils.intrinsic_euler_from_quats(
                    previous_quat, quat, self.on_quaternion_error
                )
                * self.RAD2DEG
                / self.TIMESTEP
            )
    ...
    def _showDroneLocalAxes(self, nth_drone):
        ...
        if self.GUI:
            ...
            self.X_AX[nth_drone] = self.CLIENT.addUserDebugLine(
                lineFromXYZ=[0, 0, 0],
            ...
            self.Y_AX[nth_drone] = self.CLIENT.addUserDebugLine(
                lineFromXYZ=[0, 0, 0],
            ...
            self.Z_AX[nth_drone] = self.CLIENT.addUserDebugLine(
                lineFromXYZ=[0, 0, 0],
    ...
    def _parseURDFParameters(self):
        ...
        M = sum(
            map(lambda node: float(node.get("value")), URDF_TREE.findall(".//mass"))
        )
    ...
    def _actionSpace(self):
        return spaces.Box(
            low=-np.ones((self.NUM_DRONES, 4)), high=np.ones((self.NUM_DRONES, 4))
        )
...
class SetpointWrapper(ModelableEnv, gym.Wrapper):
    ...
    def reward(self, action, obs) -> float:
        ...
        normed_closeness = tf.cast(
            1.0 - np.abs(normed_obs - normed_setpoint), tf.float32
        )
        # print(normed_closeness)
        setpoint_following = fpl.p_mean(
            fpl.smooth_constraint(normed_closeness, 0.7, 1.0), 0.0
        )
        # print(setpoint_following)
        # reward_fpl = fpl.Constraints(0.0, {
        #     "setpoint_following": setpoint_following,
        #     "action_smallness": fpl.p_mean(1.0-action, 0)**3.0
        # })
        # reward_val = fpl.fpl_scalar(reward_fpl)
        action_smallness = fpl.p_mean(1.0 - action, 0)
        # print(action_smallness)
        # reward_val = fpl.implies(setpoint_following, action_smallness)
...
if __name__ == "__main__":
    ...
    env = SetpointedAttitudeEnv(
        gui=True, test=True, urdf_path=load_assets.get_urdf_path("cf2x.urdf")
    )
    i = 0
    rw_sum = 0
    while 1:
