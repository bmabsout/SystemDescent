...
class AmazingBallEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if self.GUI:
            ...
            if self.USER_DEBUG:
                ...
                self.SLIDERS = np.ones(1)
                self.SLIDERS[0] = self.CLIENT.addUserDebugParameter("Mass", 0, 10)
    ...
    def reset_ball(self):
        self.CLIENT.resetBasePosition(self.ball_id, np.array([0,0,1]).reshape(1, 3))
        self.CLIENT.resetBaseVelocity(self.ball_id, np.array([0,0,0]))
    def reset_platform(self):
        self.CLIENT.resetBasePositionAndOrientation(self.platform_id, np.array([0,0,1]).reshape(1, 3), p.getQuaternionFromEuler([0, 0, 0]))
        self.CLIENT.resetBaseVelocity(self.platform_id, np.array([0,0,0]))
    ...
    def render(self,
        ...
        for i in range (self.NUM_DRONES):
            ...
            print(f"[INFO] BaseAviary.render() ——— platform",
    ...
    def _initialize_pybullet(self):
        ...
        self.platform_id = self.CLIENT.loadURDF("plane.urdf")
        self.ball_id = self.CLIENT.loadURDF("sphere2_rolling_friction.urdf")
