...
def generate_dataset(env: gym.Env):
    def gen_sample():
        ...
        while True:
            ...
            angle = (
                np.random.uniform(-np.pi / 7.0, np.pi / 7.0)
                + np.random.randint(2) * np.pi
            )
...
def train(batches, dynamics_model, actor, V, state_shape, args):
    ...
    def batch_value(batch):
        ...
        )  # Why V takes setpoint? only reason is to get V(setpoint)==0 ?
        ...
        zero = p_mean(
            (1.0 - V({"state": set_points, "setpoint": set_points}) ** 0.5), -1.0
        )
        ...
        angular_similarities = angular_similarity(
            tf.transpose(fxu)[0:2], tf.transpose(set_points)[0:2]
        )
